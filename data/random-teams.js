	...
	randomSet(template, slot, teamDetails) {
		...
		if (abilities[1]) {
			...
			if (template.species === 'Ambipom' && !counter['technician']) {
				...
				ability = 'Pickup';
			} else if (template.species === 'Aurorus' && ability === 'Snow Warning' && hasMove['hypervoice']) {
				...
				ability = 'Klutz';
			} else if (template.species === 'Rampardos' && !hasMove['headsmash']) {
				ability = 'Sheer Force';
		...
		// First, the extra high-priority items
			...
			item = 'Power Herb';
		} else if (hasMove['solarbeam'] && !hasAbility['Drought'] && !hasMove['sunnyday']) {
